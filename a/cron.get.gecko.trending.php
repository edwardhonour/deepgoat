...
$B=new BSCScan();
$l=$B->getGeckoTrending();
$dateTime = time();
$list=json_decode($l,true);
$counter=0;
$table_name="GOATX_RESEARCH_TRENDING";
foreach($list['coins'] as $k) {
	$n=$k['item'];
$counter++;
		$post=array();
		$t=time();
...
$sql0="UPDATE " . $table_name . " SET ";
             ...
             $sql0 .= do_one("coin_id",$n,$x[0]);
             $sql0 .= do_one("name",$n,$x[0]);
             $sql0 .= do_one("symbol",$n,$x[0]);
             $sql0 .= do_one("market_cap_rank",$n,$x[0]);
             $sql0 .= do_one("thumb",$n,$x[0]);		
             $sql0 .= do_one("small",$n,$x[0]);
             $sql0 .= do_one("large",$n,$x[0]);
             $sql0 .= do_one("slug",$n,$x[0]);
             $sql0 .= do_one("price_btc",$n,$x[0]);
             $sql0 .= do_one("score",$n,$x[0]);			 
		$sql0.=" timestamp = " . $t . " where id = '" .  strtolower($n['id']) . "'";	

...
$sql="select distinct primary_member_id, month_id, plan_code, coverage_level from apa_claims where primary_member_id <> '' order by 2";
$a=$X->sql($sql);

$iii=0;
foreach ($a as $b) { 
   $sql="select * from apa_claims where month_id = '" . $b['month_id'] . "' and primary_member_id = " . $b['primary_member_id'];
   $ff=$X->sql($sql);
   $billed=0;
   $approved=0;
   $paid=0;
   ...
   $post=array();
   $post['table_name']="apa_monthly_member_claims";
   $post['action']="insert";
   $post['member_id']=$b['primary_member_id'];
   $post['month_id']=$b['month_id'];
   ...
   $sql="select * from apa_monthly_member_claims where member_id = '" . $post['member_id'] . "' and month_id = '" . $post['month_id'] . "'";
   $h=$X->sql($sql);

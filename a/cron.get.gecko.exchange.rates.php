...
$B=new BSCScan();
$l=$B->getGeckoExchangeRates();
$dateTime = time();
$list=json_decode($l,true);
$counter=0;
$table_name="GOATX_RESEARCH_EXCHANGE_RATES";

foreach($list['rates'] as $n => $v) {
$counter++;
		$post=array();
		$t=time();
...
$sql0="UPDATE " . $table_name . " SET ";
             ...
             $sql0 .= do_one("name",$v,$x[0]);
             $sql0 .= do_one("symbol",$v,$x[0]);
             $sql0 .= do_one("unit",$v,$x[0]);
             $sql0 .= do_one("type",$v,$x[0]);			 

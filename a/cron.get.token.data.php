...
$sql="SELECT * FROM BEP20_TOKEN ORDER BY SYMBOL";
$list=$X->sql($sql);
foreach($list as $l) {
		$post=array();
		$post['TABLE_NAME']="BEP20_TOKEN";
		$post['ACTION']="insert";
		$post['ID']=$l['ID'];
		$result=$B->getBEP20TokenInfo(strtolower($l['CONTRACT_ADDRESS']));		
		...
		$X->post($post);
		$post=array();
		$post['TABLE_NAME']="BEP20_TOKEN_PRICE";
		$post['ACTION']="insert";

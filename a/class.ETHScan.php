...
class ETHScan {
	...
	function populate_raw_transactions($address,$xid) {
 			$this->populateTxInternalList($address,$xid);	
			$this->populateTxList($address,$xid);
			$this->populateTokenTxList($address,$xid);
	}
...
$l=array();
  ...
  $toPriceUSD=0;
   ...
   function get_token_historical_prices($contract,$symbol,$name, $chain="BSC") {
	...
	function get100Balances($addresses) {
		        //
			//Get ETH Balance for a single Address	
			//tested
			$url="https://api.etherscan.io/api?module=stats&action=ethprice";
                        ...
                        $url.=$list;
                        $url.="&tag=latest&apikey=QY1P3CGUEF5BMMXB2NFEQGIN5N77RC574E";
			$curl=curl_init();
			curl_setopt($curl, CURLOPT_URL, $url);
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($curl, CURLOPT_HEADER, false);
	...
	function populateTxList($address,$xid) {
			...
			if (isset($t['result'])) {
                             if (is_array($t['result'])) {
                                echo "Added " . sizeof($t['result']) . " transactions\r\n";
				foreach($t['result'] as $t0) {
					$this->txlistPostN($t0,$address,"ETH",$xid);
				}
                                if (sizeof($t['result'])>9999) {
                                    $s="INSERT INTO BIG_WALLETS (address) values ('" . $address . "')";
                                    $this->execute($s);
                                }
                            }
			}	
	...
	function populateTxInternalList($address,$xid) {
                      	
                        $startblock=0;
			$txlist=$this->getETHInternalTransactionsByAddress($address,$startblock, "99999999", "asc");
			$t=json_decode($txlist,true);
			if (isset($t['result'])) {
                                echo "Added " . sizeof($t['result']) . " internal transactions\r\n";
				foreach($t['result'] as $t0) {
					$this->internalPostN($t0,$address,"ETH",$xid);
				}
                                if (sizeof($t['result'])>9999) {
                                    $s="INSERT INTO BIG_WALLETS (address) values ('" . $address . "')";
                                    $this->execute($s);
	...
	function populateTokenTxList($address,$xid) {
		
                $startblock=0;
		$txlist=$this->getERC20TransfersByAddress($address,$startblock, "99999999", "asc");
		$t=json_decode($txlist,true);
		if (isset($t['result'])) {
                        echo "Added " . sizeof($t['result']) . " token transactions\r\n";
			foreach($t['result'] as $t0) {
				$this->tokentxPostN($t0,$address,"ETH",$xid);
			}
                                if (sizeof($t['result'])>9999) {
                                    $s="INSERT INTO BIG_WALLETS (address) values ('" . $address . "')";
                                    $this->execute($s);
...
//		}
	...
	function txlistPostN($POST, $wallet_address, $chain="BSC",$xid) {
		
	    $POST['TABLE_NAME']="ETH_RAW_TRANSACTIONS";
		...
		$output=array();
                ...
                $sql="SELECT ID FROM ETH_RAW_TRANSACTIONS WHERE hash = '" . $POST['hash'];
		...
		if ($POST['ID']==""||$POST['ID']=="0") {
                        $top = "INSERT INTO ETH_RAW_TRANSACTIONS (CREATE_TIMESTAMP, ";
                        $bottom = " VALUES (now(), ";
		        foreach ($POST as $name => $value) {
			    if ($name=="from") $name="ethFrom";
			    if ($name=="to") $name="ethTo";
			    if ($name=="value") $name="ethValue";
		            if ($name!="input"&&
			...
			$POST['ID'] = $this->db->lastInsertId();
                        ...
                        $top = "UPDATE ETH_RAW_TRANSACTIONS SET ";
		        foreach ($POST as $name => $value) {
			    if ($name=="from") $name="ethFrom";
			    if ($name=="to") $name="ethTo";
			    if ($name=="value") $name="ethValue";
	...
	function internalPostN($POST, $wallet_address, $chain="BSC",$xid) {
	...
	function tokentxPostN($POST,$wallet_address, $chain="ETH",$xid) {
